...
#if CPU == MK20D5
   #define enableUSBIrq()   NVIC_EnableIRQ(USBOTG_IRQn)
   #define disableUSBIrq()  NVIC_DisableIRQ(USBOTG_IRQn)
#elif CPU == MKL25Z4
...
//!
static void ep0StartTxTransaction( uint16_t bufSize, const uint8_t *bufPtr, uint8_t data0_1 ) {
