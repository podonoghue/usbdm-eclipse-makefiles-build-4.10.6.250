...
USBDM_ErrorCode FlashProgrammer::confirmSDID() {
   ...
   if (rc != PROGRAMMING_RC_OK) {
      Logging::error("M=0x%8.8X, V=0x%8.8X => Failed, error reading SDID, reason = %s\n",
   ...
   if (!parameters.isThisDevice(targetSDID)) {
      Logging::error("M=0x%8.8X, V=0x%8.8X => Failed (Target SDID=0x%8.8X)\n",
            parameters.getSDID().mask,
            parameters.getSDID().value,
            targetSDID);
      return PROGRAMMING_RC_ERROR_WRONG_SDID;
   }
   Logging::print("V=%8.8X => OK\n", targetSDID);
   return PROGRAMMING_RC_OK;
}
...
USBDM_ErrorCode FlashProgrammer::loadLargeTargetProgram(memoryElementType    *buffer,
   ...
   if (targetProgramInfo.maxDataSize<40) {
      Logging::error("Data buffer is too small - 0x%X \n", targetProgramInfo.maxDataSize);
      return PROGRAMMING_RC_ERROR_INTERNAL_CHECK_FAILED;
   }
#if TARGET == MC56F80xx
...
USBDM_ErrorCode FlashProgrammer::probeMemory(MemorySpace_t memorySpace, uint32_t address) {
   ...
   if (rc != BDM_RC_OK) {
      Logging::error("RAM memory probe failed @%s:0x%08X, rc = %d (%s)\n", getMemSpaceName(memorySpace), address, rc, USBDM_GetErrorString(rc));
      return rc;
   }
   return PROGRAMMING_RC_OK;
}
...
USBDM_ErrorCode FlashProgrammer::executeTargetProgram(memoryElementType *pBuffer, uint32_t dataSize) {
   ...
   Logging::print("        PC       Instr'n      R0       SP  \n"
         "=============================================\n");
   ...
   for (int i=0;;i++) {
      ...
      if (i == maxtrace)
         break;
      Logging::print("%3d: 0x%06X: 0x%04X %04X 0x%06X 0x%06X \n",
              i, pcValue, getData16Le(instruction), getData16Le(instruction+2), r0Value, spValue);
      if (DSC_TargetStep() != BDM_RC_OK) {
         Logging::print("FlashProgrammer::executeTargetProgram() - TargetStep() failed\n");
...
USBDM_ErrorCode FlashProgrammer::setFlashSecurity(FlashImage &flashImage, MemoryRegionConstPtr flashRegion) {
   ...
   uint8_t memory[size];
   ReadMemory(MS_PWord, size, securityAddress, memory);
...
#if (TARGET == MC56F80xx)
      Logging::printDump(data, size, securityAddress, WORD_ADDRESS);
#else
