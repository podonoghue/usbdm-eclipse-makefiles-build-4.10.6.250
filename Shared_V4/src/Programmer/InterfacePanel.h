...
class InterfacePanel : public wxPanel {
   ...
   DECLARE_CLASS( InterfacePanel )
   DECLARE_EVENT_TABLE()

public:
   InterfacePanel(wxWindow* parent, SharedPtr shared);
   InterfacePanel(Shared *shared);

   ~InterfacePanel() {
//      print("~InterfacePanel\n");
   }
...
private:
   ...
   DeviceDataPtr           &currentDevice;      // Information about currently selected device
#endif
//   std::string             &selectedBdm;     // Information about currently selected BDM
   ...
   vector<BdmInformation>  connectedBDMs;      // Table of connected BDMs
   wxString                bdmIdentification;
   int                     bdmDeviceNum;

   SharedPtr               shared;
   TargetOptions           targetProperties;  // Flag indicating required target specific dialogue elements
};
