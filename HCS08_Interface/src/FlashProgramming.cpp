...
USBDM_ErrorCode FlashProgrammer::readTargetChipId(uint32_t *targetSDID, bool doInit) {
   ...
   *targetSDID = 0x0000;
   ...
   if (ReadMemory(SDIDLength, SDIDLength, parameters.getSDIDAddress(), SDIDValue) !=  BDM_RC_OK) {
      Logging::error("A=0x%X - Failed ReadMemory()\n", parameters.getSDIDAddress());
...
USBDM_ErrorCode FlashProgrammer::initialiseTargetFlash() {
   ...
   Logging::print("Target Bus Frequency = %ud kHz\n", targetBusFrequency);
#endif
...
//! @note - see loadTargetProgram(MemoryRegionConstPtr, FlashOperation) for details
//!
USBDM_ErrorCode FlashProgrammer::loadTargetProgram(MemoryRegionConstPtr memoryRegionPtr, FlashOperation flashOperation) {
   FlashProgramConstPtr flashProgram = memoryRegionPtr->getFlashprogram();
   if (!flashProgram) {
      // Try to get device general routines
      flashProgram = parameters.getCommonFlashProgram();
   }
   if (!flashProgram) {
...
USBDM_ErrorCode FlashProgrammer::loadTargetProgram(FlashProgramConstPtr flashProgram, FlashOperation flashOperation) {
   LOGGING;
   memoryElementType     buffer[4000] = {0};
...
USBDM_ErrorCode FlashProgrammer::loadLargeTargetProgram(memoryElementType    *buffer,
   ...
   if (currentFlashProgram != flashProgram)  {
      ...
      USBDM_ErrorCode rc = WriteMemory(memorySpace, codeLoadSize, codeLoadAddress, (uint8_t *)buffer);
      if (rc != BDM_RC_OK) {
         Logging::print("WriteMemory() Failed, rc = %d (%s)\n", rc, USBDM_GetErrorString(rc));
...
USBDM_ErrorCode FlashProgrammer::loadSmallTargetProgram(memoryElementType    *buffer,
   ...
   uint32_t massErase      = headerPtr->massErase;
   uint32_t blankCheck     = headerPtr->blankCheck;
   uint32_t selectiveErase = headerPtr->selectiveErase;
   uint32_t verify         = headerPtr->verify;

   Logging::print("Loaded Image (unmodified) :\n");
   Logging::print("   flashProgramHeader program         = 0x%08X\n",     program);
   Logging::print("   flashProgramHeader.massErase       = 0x%08X\n",     massErase);
   Logging::print("   flashProgramHeader.blankCheck      = 0x%08X\n",     blankCheck);
   Logging::print("   flashProgramHeader.selectiveErase  = 0x%08X\n",     selectiveErase);
   Logging::print("   flashProgramHeader.verify          = 0x%08X\n",     verify);
   Logging::print("   flashProgramHeader.loadAddress     = 0x%08X\n",     loadAddress);
   ...
   // Determine location and size of required routine
   ...
   case OpNone:
      return PROGRAMMING_RC_ERROR_INTERNAL_CHECK_FAILED;
   case OpProgram:
      codeStart = program;        codeEnd = massErase;      break;
   case OpBlockErase:
      codeStart = massErase;      codeEnd = blankCheck;     break;
   case OpBlankCheck:
      codeStart = blankCheck;     codeEnd = selectiveErase; break;
   case OpSelectiveErase:
      codeStart = selectiveErase; codeEnd = verify;         break;
   case OpVerify:
      codeStart = verify;         codeEnd = size;           break;
   case OpWriteRam:
   default:
      Logging::error("flashOperation is invalid.\n");
      return PROGRAMMING_RC_ERROR_ILLEGAL_PARAMS;
   }
   if (codeStart <= 0) {
      Logging::error("codeStart address is invalid.\n");
      return PROGRAMMING_RC_ERROR_ILLEGAL_PARAMS;
   }
   if (codeEnd <= 0) {
      codeEnd = size;
   ...
   uint32_t codeLoadAddress = parameters.getRamEnd() - codeLoadSize + 1;

   // Location of data buffer
   uint32_t dataAddress     = loadAddress+program;

   // Save location of entry point
   targetProgramInfo.entry         = codeLoadAddress;
   // Were to load flash buffer (including header)
   targetProgramInfo.headerAddress = loadAddress;
   // Save location of RAM data buffer
   targetProgramInfo.dataOffset    = program;
   // Save maximum size of the buffer (in memoryElementType)
...
//   targetProgramInfo.usePagedAddresses = (program>=sizeof(SmallTargetFlashDataHeader));
   ...
   Logging::print("RAM buffer[0x%04X...0x%04X]\n",
         targetProgramInfo.headerAddress+targetProgramInfo.dataOffset, targetProgramInfo.entry);
   Logging::print("Program code[0x%04X...0x%04X]\n",
         targetProgramInfo.entry, parameters.getRamEnd());

   // Sanity check buffer
   if (((uint32_t)(targetProgramInfo.headerAddress+targetProgramInfo.dataOffset)<parameters.getRamStart()) ||
       ((uint32_t)(targetProgramInfo.headerAddress+targetProgramInfo.dataOffset+targetProgramInfo.maxDataSize-1)>parameters.getRamEnd())) {
      Logging::print("Data buffer location [0x%06X..0x%06X] is outside target RAM [0x%06X-0x%06X]\n",
            targetProgramInfo.headerAddress+targetProgramInfo.dataOffset, targetProgramInfo.entry, parameters.getRamStart(), parameters.getRamEnd());
      return PROGRAMMING_RC_ERROR_INTERNAL_CHECK_FAILED;
   }
   if (targetProgramInfo.maxDataSize<40) {
      Logging::print("Data buffer is too small - 0x%X \n", targetProgramInfo.maxDataSize);
      return PROGRAMMING_RC_ERROR_INTERNAL_CHECK_FAILED;
   }
#if TARGET == MC56F80xx
   MemorySpace_t memorySpace = MS_PWord;
...
#endif
   // Write the flash programming code to target memory
   if (WriteMemory(memorySpace, codeLoadSize, codeLoadAddress, buffer+codeStart) != BDM_RC_OK) {
      return PROGRAMMING_RC_ERROR_BDM_WRITE;
   }
   currentFlashProgram   = flashProgram;
   currentFlashOperation = flashOperation;
   ...
   // Loaded routines do not support extended operations
   targetProgramInfo.programOperation = DO_PROGRAM_RANGE;
   return BDM_RC_OK;
}
...
USBDM_ErrorCode FlashProgrammer::probeMemory(MemorySpace_t memorySpace, uint32_t address) {
   ...
   if (savedDataValid) {
      WriteMemory(memorySpace,memorySpace&MS_SIZE,address,savedData);
   }

   if (rc != BDM_RC_OK) {
      Logging::error("RAM memory probe failed @0x%08X, rc = %d (%s)\n", address, rc, USBDM_GetErrorString(rc));
      return rc;
   }
   return PROGRAMMING_RC_OK;
}
...
USBDM_ErrorCode FlashProgrammer::initSmallTargetBuffer(memoryElementType *buffer) {
   ...
   if (targetProgramInfo.useWatchdogAddress) {
      ...
      pFlashHeader->page_wdog_Address = nativeToTarget16(parameters.getWatchdogAddress());
   }
   ...
   case OpNone:
   default:
      Logging::error("Error: Unsupported operation\n");
      return PROGRAMMING_RC_ERROR_INTERNAL_CHECK_FAILED;
   case OpSelectiveErase:
   {
      uint32_t sectorSize = flashOperationInfo.sectorSize;
      uint32_t endAddress = flashOperationInfo.flashAddress+flashOperationInfo.dataSize-1;
      uint32_t numSectors = 1+trunc(endAddress/sectorSize)-trunc(flashOperationInfo.flashAddress/sectorSize);
      Logging::print("numSectors = %d\n", numSectors);
      pFlashHeader->dataSize_sectorCount   = nativeToTarget16(numSectors);
      pFlashHeader->dataAddress_sectorSize = nativeToTarget16(sectorSize);
   }
      break;
   case OpProgram:
   case OpVerify:
   case OpBlankCheck:
      pFlashHeader->dataAddress_sectorSize  = nativeToTarget16(targetProgramInfo.headerAddress+targetProgramInfo.dataOffset);
      pFlashHeader->dataSize_sectorCount    = nativeToTarget16(flashOperationInfo.dataSize);
      break;
   case OpBlockErase:
      pFlashHeader->dataAddress_sectorSize  = 0;
...
//         report("FlashProgrammer::executeTargetProgram()");
   ...
   do {
      ...
      progressTimer->progress(0, NULL);
   } while (((runStatus&HC08_BDCSCR_BDMACT) == 0) && (--timeout>0));
   Logging::printq("\n");
   TargetHalt();
   unsigned long value;
   USBDM_Connect();
   ReadPC(&value);
   Logging::print("Start PC = 0x%08lX, end PC = 0x%08lX\n", (unsigned long)targetRegPC, (unsigned long)value);
...
USBDM_ErrorCode FlashProgrammer::setFlashSecurity(FlashImage &flashImage, MemoryRegionConstPtr flashRegion) {
   ...
   if (memcmp(data, memory, size) == 0) {
      ...
      Logging::print("Setting security region, \n"
            "              mem[0x%06lX-0x%06lX] = \n", (unsigned long)securityAddress, (unsigned long)(securityAddress+size/sizeof(memoryElementType)-1));
...
USBDM_ErrorCode FlashProgrammer::runTCLScript(TclScriptConstPtr script) {
   ...
   if (rc != 0) {
      ...
      Logging::error("%s", script->toString().c_str());
      return PROGRAMMING_RC_ERROR_INTERNAL_CHECK_FAILED;
   }
   return PROGRAMMING_RC_OK;
}
...
USBDM_ErrorCode FlashProgrammer::doFlashBlock(FlashImage     *flashImage,
   ...
   MemoryRegionConstPtr memoryRegionPtr = parameters.getMemoryRegionFor(flashAddress&memoryAddressMask, memorySpace);
   if (memoryRegionPtr == NULL) {
   ...
   if (!memoryRegionPtr->findLastContiguous(flashAddress&memoryAddressMask, &lastContiguous, memorySpace)) {
      ...
      Logging::error("Block %s[0x%06X...] is within target memory BUT location not found!.\n", getMemSpaceName(memorySpace), flashAddress&memoryAddressMask);
      return PROGRAMMING_RC_ERROR_INTERNAL_CHECK_FAILED;
   }
   ...
   if (memoryRegionPtr->isProgrammableMemory()) {
      ...
      switch (flashOperation) {
         ...
         case OpWriteRam:
            ...
            Logging::print("  ...Skipping block %s[0x%06X..0x%06X] as ROM memory assumed processed in earlier phase\n",
         ...
         default:
            // Unexpected
            Logging::print("  ...Unexpected flashOperation\n");
            return PROGRAMMING_RC_ERROR_INTERNAL_CHECK_FAILED;
      }
   }
       ...
       */
         ...
         case OpSelectiveErase:
            ...
            Logging::print("  ...Skipping block %s[0x%06X..0x%06X] as RAM memory assumed processed in later phase\n",
                  memoryRegionPtr->getMemoryTypeName(), (flashAddress&memoryAddressMask), (flashAddress&memoryAddressMask)+blockSize-1);
            // No further processing of this block
            flashAddress += blockSize;
            return BDM_RC_OK;
         ...
         default:
            // Unexpected
            Logging::print("  ...Unexpected ramOperation\n");
            return PROGRAMMING_RC_ERROR_INTERNAL_CHECK_FAILED;
         }
   }
   else {
      // Unexpected
      Logging::error("Block %s[0x%06X..0x%06X] is not within target memory\n",
            memoryRegionPtr->getMemoryTypeName(), (flashAddress&memoryAddressMask), (flashAddress&memoryAddressMask)+blockSize-1);
      return PROGRAMMING_RC_ERROR_OUTSIDE_TARGET_FLASH;
   }
   ...
   memoryElementType  buffer[100+maxSplitBlockSize];
   ...
   while (blockSize>0) {
      ...
      if (flashOperation == OpWriteRam) {
          ...
          * RAM Writes
          */
         Logging::print("ramBlock splitBlock %s, %s[0x%06X..0x%06X]\n",
               MemoryRegion::getMemoryTypeName(memoryType), getMemSpaceName(memorySpace), targetAddress, targetAddress+splitBlockSize-1);
#if (TARGET == ARM)
         rc = WriteMemory(MS_Long, splitBlockSize*sizeof(memoryElementType), targetAddress, (const uint8_t *)bufferData);
#elif (TARGET == MC56F80xx)
         rc = WriteMemory(memorySpace, splitBlockSize*sizeof(memoryElementType), targetAddress, (const uint8_t *)bufferData);
//         Logging::printDump((const uint8_t *)bufferData, splitBlockSize*sizeof(memoryElementType), targetAddress, WORD_ADDRESS);
#else
         rc = WriteMemory(MS_Word, splitBlockSize*sizeof(memoryElementType), targetAddress, (const uint8_t *)bufferData);
#endif
      }
      else {
         /*
          * Flash writes
          */
         Logging::print("splitBlock %s[0x%06X..0x%06X]\n",
...
USBDM_ErrorCode FlashProgrammer::doReadbackVerify(FlashImage *flashImage) {
   ...
   while (enumerator->isValid()) {
      ...
      while (regionSize>0) {
         ...
         if (memRegion == NULL) {
            Logging::error("Verifying Block %s[0x%8.8X..0x%8.8X] - Not in valid memory region\n", getMemSpaceName(memorySpace), memoryAddress, memoryAddress+regionSize-1);
            return PROGRAMMING_RC_ERROR_OUTSIDE_TARGET_FLASH;
         }
#if (TARGET == ARM)
...
#endif
         if (ReadMemory(memorySpace, blockSize*sizeof(memoryElementType), memoryAddress, (uint8_t *)buffer) != BDM_RC_OK) {
            return PROGRAMMING_RC_ERROR_BDM_READ;
         }
         blockResult = TRUE;
         uint32_t testIndex;
...
//                     buffer[testIndex]);
         ...
         Logging::print("Verifying Sub-block %s[0x%8.8X..0x%8.8X] => %s\n", getMemSpaceName(memorySpace), memoryAddress, memoryAddress+blockSize-1, blockResult?"OK":"FAIL");
         if (!blockResult) {
            Logging::printDump((const uint8_t *)buffer, blockSize*sizeof(memoryElementType), memoryAddress);
         }
