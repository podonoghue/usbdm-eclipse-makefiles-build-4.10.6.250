...
#endif
static const TargetType_t targetType = T_HCS08;
...
const static DiFeaturesT diFeatures = {
  /* .szIdentification                 = */ (DiStringT)"USBDM HCS08",
 ...
 */
   if (bdmOptions.maskInterrupts) {
      Logging::print("DiExecSingleStep() - checking if interrupt masking needed\n");
      ...
      if ((ccrValue&interruptMask) != 0) {
         ...
         // Mask interrupts
         Logging::print("DiExecSingleStep() - masking interrupts\n");
         ...
         switch(currentOpcode) {
            ...
            case stopOpcode :
               Logging::print("DiExecSingleStep() - skipping CCR restore\n");
               // Don't 'fix' CCR as updated by instruction or int ack
               break;
            case tpaOpcode :
               // Fix A & CCR  (clear I flag)
               Logging::print("DiExecSingleStep() - fixing A & CCR reg\n");
            ...
            default :
               ...
               // Unmask interrupts
               Logging::print("DiExecSingleStep() - fixing CCR reg\n");
               USBDM_ReadReg(HCS08_RegCCR, &ccrValue);
               USBDM_WriteReg(HCS08_RegCCR, ccrValue&~interruptMask);
               break;
         }
...
//      return setErrorState(DI_ERR_NONFATAL, "Connection with target lost");
   ...
   if ((status&HC08_BDCSCR_BDMACT) != 0) {
