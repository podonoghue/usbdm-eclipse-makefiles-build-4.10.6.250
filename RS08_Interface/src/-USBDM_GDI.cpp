...
#endif
static const TargetType_t targetType = T_RS08;
   ...
   while (dnBufferItems>0) {
      ...
      if (flashImage != NULL) {
         ...
         if ((memoryType == MemEEPROM) || (memoryType == MemFLASH) || (memoryType == MemPROM)) {
            USBDM_ErrorCode rc = flashImage->loadData(sub, writeAddress, memoryReadWriteBuffer);
//         Logging::print("loadData - address=0x%06X, size=0x%04X\n", (uint32_t)address, blockSize);
//         Logging::printDump(memoryReadWriteBuffer,blockSize,address,size);
            writeDone = true;
            if (rc != BDM_RC_OK) {
               return setErrorState(DI_ERR_FATAL, rc);
            }
            ...
            Logging::print("Warning: unsupported memory write during Flash programming (ty=%s, sp=%s, 0x%06X...0x%06X)\n",
                  MemoryRegion::getMemoryTypeName(memoryType), getMemSpaceName(memorySpace), (uint32_t)address, (uint32_t)address+sub-1);
         }
      }
#endif
      if (!writeDone) {
...
USBDM_ErrorCode rc = BDM_RC_OK;
   ...
   switch (dnRegNumber) {
      case  0 : /* PC  */ rc = USBDM_ReadReg(RS08_RegCCR_PC, &value);
                          value &= 0x3FFF;
                          break;
      case  1 : /* A   */ rc = USBDM_ReadReg(RS08_RegA,      &value);   break;
      case  3 : /* SPC */ rc = USBDM_ReadReg(RS08_RegSPC,    &value);   break;
      case  5 : /* CCR */ rc = USBDM_ReadReg(RS08_RegCCR_PC, &value);
                          value = (value>>14) & 0x3;
                          break;
      default : /*   ? */ return setErrorState(DI_ERR_PARAM, ("Illegal register identifier"));
   }
   ...
   *drvValue = (U32c)value;
   Logging::print("DiRegisterRead(0x%X(%d) => 0x%08X)\n", dnRegNumber, dnRegNumber, value);
   return setErrorState(DI_OK);
}
...
DiReturnT DiBreakpointSet ( DiBpResultT *pdnBreakpointId,
   ...
   if (breakpointId != 0) {
      Logging::print("DiBreakpointSet(0x%4X) - Too many breakpoints\n", (uint32_t)bkptAddress);
   ...
   Logging::print("DiBreakpointSet(0x%4X, #%d:%s)\n",
...
unsigned long bdmscrValue;
   ...
   Logging::print("DiBreakpointClear()\n");
...
DiReturnT DiBreakpointClearAll ( void ) {
   ...
   Logging::print("DiBreakpointClearAll()\n");
...
//   pdesExitStatus->szReason = (DiStringT)getBDMStatusName(&USBDMStatus);
   ...
   if (USBDMStatus.connection_state == SPEED_NO_INFO) {
      Logging::print("DiExecGetStatus()=> NO_INFO\n");
      return setErrorState(DI_OK);
...
//      return setErrorState(DI_ERR_NONFATAL, "Connection with target lost");
   ...
   if (BDMrc != BDM_RC_OK) {
      Logging::print("DiExecGetStatus()=> Status read failed\n");
      return setErrorState(DI_OK);
//      return setErrorState(DI_ERR_NONFATAL, BDMrc);
   }
   if ((status&RS08_BDCSCR_ENBDM) == 0) {
      Logging::print("DiExecGetStatus()=> ENBDM=0\n");
      return setErrorState(DI_OK);
