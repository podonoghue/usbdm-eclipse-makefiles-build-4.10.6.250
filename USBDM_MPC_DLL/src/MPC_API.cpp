/*! \file
    \brief MPC EONCE debugging interface
...
const static EonceRegisterDetails_t eonceRegisterDetails[] = {
   /*  0 */ {JTAG_DID_ADDRESS, 32,   "JTAG_DID" },
   /*  1 */ {CPUSCR_ADDRESS  , 6*32, "CPUSCR"   },
   /*  2 */ {OCR_ADDRESS     , 32,   "OCR"      },
   /*  3 */ {IAC1_ADDRESS    , 32,   "IAC1"     },
   /*  4 */ {IAC2_ADDRESS    , 32,   "IAC2"     },
   /*  5 */ {IAC3_ADDRESS    , 32,   "IAC3"     },
   /*  6 */ {IAC4_ADDRESS    , 32,   "IAC4"     },
   /*  7 */ {DAC1_ADDRESS    , 32,   "DAC1"     },
   /*  8 */ {DAC2_ADDRESS    , 32,   "DAC2"     },
   /*  9 */ {DVC1_ADDRESS    , 32,   "DVC1"     },
   /* 10 */ {DVC2_ADDRESS    , 32,   "DVC2"     },
   /* 11 */ {IAC5_ADDRESS    , 32,   "IAC5"     },
   /* 12 */ {IAC6_ADDRESS    , 32,   "IAC6"     },
   /* 13 */ {IAC7_ADDRESS    , 32,   "IAC7"     },
   /* 14 */ {IAC8_ADDRESS    , 32,   "IAC8"     },
   /* 15 */ {DBCNT_ADDRESS   , 32,   "DBCNT"    },
   /* 16 */ {PCFIFO_ADDRESS  , 32,   "PCFIFO"   },
   /* 17 */ {EDBCR0_ADDRESS  , 32,   "EDBCR0"   },
   /* 18 */ {EDBSR0_ADDRESS  , 32,   "EDBSR0"   },
   /* 19 */ {DBSR_ADDRESS    , 32,   "DBSR"     },
   /* 20 */ {DBCR0_ADDRESS   , 32,   "DBCR0"    },
   /* 21 */ {DBCR1_ADDRESS   , 32,   "DBCR1"    },
   /* 22 */ {DBCR2_ADDRESS   , 32,   "DBCR2"    },
   /* 23 */ {DBCR3_ADDRESS   , 32,   "DBCR3"    },
   /* 24 */ {DBCR4_ADDRESS   , 32,   "DBCR4"    },
   /* 25 */ {DBCR5_ADDRESS   , 32,   "DBCR5"    },
   /* 26 */ {DBCR6_ADDRESS   , 32,   "DBCR6"    },
   /* 27 */ {DDAM_ADDRESS    , 32,   "DDAM"     },
   /* 28 */ {DEVENT_ADDRESS  , 32,   "DEVENT"   },
   /* 29 */ {DBERC0_ADDRESS  , 32,   "DBERC0"   },
   /* 30 */ {CDACNTL_ADDRESS , 32,   "CDACNTL"  },
   /* 31 */ {CDADATA_ADDRESS , 32,   "CDADATA"  },
   /* 32 */ {NX3ACC_ADDRESS  , 32,   "NX3ACC"   },
   /* 33 */ {LSRL_ADDRESS    , 32,   "LSRL"     },
   /* 34 */ {EnONCE_ADDRESS  , 32,   "EnONCE"   },
   /* 35 */ {BYPASS_ADDRESS  , 32,   "BYPASS"   },
};

#ifdef LOG_JTAG
   #define TEST_LOGIC_RESET_RECOVERY_NOPS (1)
#else
   #define TEST_LOGIC_RESET_RECOVERY_NOPS (50) // ~300 us
#endif
...
uint8_t readCoreIdCodeSequence[] = {
   ...
   // Wait a while ~300 us
   JTAG_REPEAT_Q(TEST_LOGIC_RESET_RECOVERY_NOPS),
      JTAG_NOP,
   JTAG_END_REPEAT,
   // Write IDCODE command to IR
   JTAG_MOVE_IR_SCAN,
   JTAG_SET_EXIT_SHIFT_DR,
   JTAG_SHIFT_OUT_Q(JTAG_MASTER_COMMAND_LENGTH), JTAG_IDCODE_COMMAND,
   // Read IDCODE from DR
   JTAG_SET_EXIT_IDLE,
   JTAG_SHIFT_IN_Q(32),
   JTAG_END
...
const static uint8_t selectCoreTapSequence[] = {
   // Reset TAP
   JTAG_TEST_LOGIC_RESET,
   // Wait recover time ~2.26ms
//   JTAG_REPEAT_16(TEST_LOGIC_RESET_RECOVERY_NOPS),
//      JTAG_NOP,
//   JTAG_END_REPEAT,
   // Write Enable ONCE TAP command to IR
   JTAG_MOVE_IR_SCAN,
   JTAG_SET_EXIT_IDLE,
   JTAG_SHIFT_OUT_Q(JTAG_MASTER_COMMAND_LENGTH), ACCESS_AUX_TAP_ONCE_COMMAND,
   JTAG_END,
};
   USBDM_ErrorCode rc;
...
JTAG32 regData(0,32);
   ...
   uint16_t command = eonceRegisterDetails[regNo].address|ONCE_CMD_READ;
   uint8_t  length  = eonceRegisterDetails[regNo].length;

   uint8_t readRegisterSequence[] = {
      JTAG_MOVE_IR_SCAN,                          // Access ONCE (IR-CHAIN)
      JTAG_SET_EXIT_SHIFT_DR,
      JTAG_SHIFT_OUT_Q(ONCE_CMD_LENGTH), JTAG_DATA_16(command), // ONCE command - Write register+RegNo
      JTAG_SET_EXIT_IDLE,
...
//! @note Leaves Core TAP in RUN-TEST/IDLE
//!
USBDM_ErrorCode writeONCEReg(int regNo, uint32_t regValue, uint8_t modifiers) {
   USBDM_ErrorCode rc;
...
/*  2 */ JTAG_SHIFT_OUT_Q(ONCE_CMD_LENGTH), JTAG_DATA_16(command), // ONCE command - Write register+RegNo+modifier
/*  5 */ JTAG_SET_EXIT_IDLE,
/*  6 */ JTAG_SHIFT_OUT_Q(length),     // Shift-out data value
/*  7 */ JTAG_NOP,   // 4-byte space for value to write (1-32 bits)
         JTAG_NOP,
         JTAG_NOP,
         JTAG_NOP,
   ...
   // Copy value to write
   regData.copyToArray(writeRegisterSequence+7);
...
void Be32ToByte(uint32_t source, uint8_t destination[]) {
   destination[0] = source>>24;
   destination[1] = source>>16;
...
/*  0    */ JTAG_MOVE_IR_SCAN,
/*  1    */ JTAG_SET_EXIT_SHIFT_DR,
/* 2,3,4 */ JTAG_SHIFT_OUT_Q(ONCE_CMD_LENGTH), JTAG_DATA_16(command), // ONCE command - Write register+RegNo+modifier
/*  5    */ JTAG_SET_EXIT_IDLE,
/* 6,7   */ JTAG_SHIFT_OUT_D(length),     // Shift-out data value
/*  8    */ JTAG_END, // Terminate sequence
            // Data
/* 6*4   */ JTAG_NOP, JTAG_NOP, JTAG_NOP, JTAG_NOP,
            JTAG_NOP, JTAG_NOP, JTAG_NOP, JTAG_NOP,
            JTAG_NOP, JTAG_NOP, JTAG_NOP, JTAG_NOP,
            JTAG_NOP, JTAG_NOP, JTAG_NOP, JTAG_NOP,
            JTAG_NOP, JTAG_NOP, JTAG_NOP, JTAG_NOP,
            JTAG_NOP, JTAG_NOP, JTAG_NOP, JTAG_NOP,
   ...
   // Copy value to write
   uint8_t *ptr = writeRegisterSequence+9;
   Be32ToByte(regValues->CTL,   ptr); ptr += 4;
   Be32ToByte(regValues->IR,    ptr); ptr += 4;
   Be32ToByte(regValues->PC,    ptr); ptr += 4;
   Be32ToByte(regValues->MSR,   ptr); ptr += 4;
   Be32ToByte(regValues->WBBRH, ptr); ptr += 4;
   Be32ToByte(regValues->WBBRL, ptr); ptr += 4;
...
USBDM_ErrorCode rc;
   ...
   uint16_t command = eonceRegisterDetails[1].address|ONCE_CMD_READ;
   uint8_t  length  = eonceRegisterDetails[1].length;
   ...
   uint8_t readRegisterSequence[] = {
      ...
      JTAG_SHIFT_OUT_Q(ONCE_CMD_LENGTH), JTAG_DATA_16(command), // ONCE command - Write register+RegNo
      JTAG_SET_EXIT_IDLE,
      JTAG_SHIFT_IN_D(length),                    // Shift-in data value
      JTAG_END
   };
   ...
   regValues->CTL   = (uint32_t)JTAG32(outPtr, 32);
   regValues->IR    = (uint32_t)JTAG32(outPtr, 32);
   regValues->PC    = (uint32_t)JTAG32(outPtr, 32);
...
JTAG32 regData(0,32);
   ...
   uint16_t command = NOREG_ADDRESS|ONCE_CMD_READ;

   uint8_t readRegisterSequence[] = {
      JTAG_MOVE_IR_SCAN,                          // Access ONCE (IR-CHAIN)
      JTAG_SET_EXIT_IDLE,
      JTAG_SHIFT_IN_OUT_Q(ONCE_CMD_LENGTH), JTAG_DATA_16(command), // ONCE command - Write register+RegNo
      JTAG_END
   };
   // Read EONCE register
   rc = executeJTAGSequence(sizeof(readRegisterSequence), readRegisterSequence,
...
sequence_type targetReadRegSequence[DSC_CoreRegisterCount] = {
...
sequence_type targetWriteRegSequence[DSC_CoreRegisterCount] = {
...
// Cached core IDCODE
uint32_t coreIdcode = 0xFFFFFFFF;
...
//!
dscInfo_t dscInfo = {sizeof(dscInfo_t), 100, 100};
...
static const uint8_t enableEONCESequence[] = {
   ...
   JTAG_SET_EXIT_IDLE,
   JTAG_SHIFT_IN_OUT_Q(JTAG_CORE_COMMAND_LENGTH), ACCESS_AUX_TAP_ONCE_COMMAND,
   JTAG_END
};
JTAG32 coreStatus(0,JTAG_CORE_COMMAND_LENGTH);
USBDM_ErrorCode rc;
