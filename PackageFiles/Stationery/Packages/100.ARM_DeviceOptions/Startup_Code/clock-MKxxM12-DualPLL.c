...
//   PORTA_PCR19 = PORT_PCR_MUX(0);
   ...
   OSC0_CR    = OSC0_CR_ERCLKEN_M|OSC0_CR_EREFSTEN_M|OSC0_CR_SCP_M;
...
//   PORTE_PCR25 = PORT_PCR_MUX(0);
   ...
   MCG_C7 = MCG_C7_OSCSEL_M; // OSCSEL = 0,1 -> XTAL/XTAL32
   ...
   MCG_C10 = MCG_C10_LOCRE2_M     | // LOCRE1 = 0,1   -> Loss of clock reset enable
...
#define BYPASS (2) // CLKS value used while PLL locks
   ...
   do {
      __asm__("nop");
   } while ((MCG_S2 & MCG_S2_OSCINIT1_MASK) == 0);
#endif
   ...
   // =============================================================
   MCG_C5 =   MCG_C5_PLLREFSEL0_M   |  // PLLREFSEL0 = 0,1 -> OSC0/OSC1 select
   ...
   // =============================================================
   MCG_C11 =  MCG_C11_PLLREFSEL1_M   |  // PLLREFSEL1 = 0,1 -> OSC0/OSC1 select
   ...
   // Set up PLL0
   // =============================================================
   MCG_C6 = MCG_C6_LOLIE0_M    |  // LOLIE0 = 0,1 -> Loss of Lock interrupt
   ...
   // Set up PLL1
   // =============================================================
   MCG_C12 = MCG_C12_LOLIE1_M    |  // LOLIE1 = 0,1 -> Loss of Lock interrupt
   ...
   do {
      __asm__("nop");
   } while((MCG_S2 & MCG_S2_LOCK1_MASK) == 0);
#endif
...
void SystemCoreClockUpdate(void) {
   ...
   switch (MCG_S&MCG_S_CLKST_MASK) {
      ...
      case MCG_S_CLKST(3) : // PLL
         if ((MCG_C11&MCG_C11_PLLCS_MASK) == 0) {
            // PLL0
            if ((MCG_C11&MCG_C11_PLLREFSEL1_MASK)==0) {
               // OCS0
            ...
            // PLL1
            if ((MCG_C5&MCG_C5_PLLREFSEL0_MASK)==0) {
               // OCS0
               oscerclk = OSCCLK0_CLOCK;
            }
            else {
