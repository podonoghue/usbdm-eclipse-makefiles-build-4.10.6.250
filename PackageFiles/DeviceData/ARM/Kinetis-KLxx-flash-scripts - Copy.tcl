...
proc massEraseTarget { } {
   ...
   set mdmApControl 0
   catch { set mdmApControl [rcreg $::MDM_AP_Control] }
   for {set retry 0} {$retry < 10} {incr i} {
      ...
      if { [catch { wcreg $::MDM_AP_Control $mdmApControl } ] } {
      ...
      if [expr (($mdmApControl & $::MDM_AP_C_SYSTEM_RESET) == 0)] {
         ;# puts "MDM_AP_C_SYSTEM_RESET failed"
         continue;
      }
   ...
   for {set retry 0} {$retry < 20} {incr i} {
      set mdmApControl [rcreg $::MDM_AP_Control]
      if [expr (($mdmApControl & $::MDM_AP_C_MASS_ERASE) == 0)] {
         ;# puts "MDM_AP_C_MASS_ERASE success"
         break;
      }
   }
   ;# Release external hardware reset
   pinSet
